...
const IndicatorOptions = [
    { key: "mask", itemName: "マスク着用率" },
    { key: "mask_work_outside_home_1d", itemName: "マスク着用率（職場）" },
    { key: "mask_shop_1d", itemName: "マスク着用率（買い物）" },
    { key: "mask_restaurant_1d", itemName: "マスク着用率（飲食店）" },
    { key: "mask_spent_time_1d", itemName: "マスク着用率マスク着用率（同居人以外と過ごすとき）" },
    { key: "mask_large_event_1d", itemName: "マスク着用率（大規模イベント）" },
    { key: "mask_public_transit_1d", itemName: "マスク着用率（公共交通機関）" },
    { key: "activity_large_event", itemName: "大規模イベント参加率" },
    { key: "activity_public_transit", itemName: "公共交通機関利用率" },
    { key: "activity_shop", itemName: "買い物をした人の割合" },
    { key: "activity_spent_time", itemName: "同居人以外と過ごした人の割合" },
    { key: "activity_work_outside_home", itemName: "職場へ出社した割合" },
    { key: "worried_catch_covid", itemName: "コロナを不安に感じている人の割合" },
    { key: "belief_distancing_effective", itemName: "ソーシャルディスタンスが効果的だと考えている人の割合" },
    { key: "belief_masking_effective", itemName: "マスクが効果的だと考えている人の割合" },
    { key: "others_distanced_public", itemName: "他人のソーシャルディスタンス率" },
    { key: "others_masked_public", itemName: "他人のマスク着用率" },
    { key: "belief_vaccinated_mask_unnecessary", itemName: "ワクチン接種していればマスクが不要と考える人の割合" },
    { key: "belief_children_immune", itemName: "子供はコロナにかからないと考える人の割合" },
    { key: "belief_no_spread_hot_humid", itemName: "高温多湿な環境であればコロナは感染拡大しないと考える人の割合" },
    { key: "received_news_govt_health", itemName: "政府から発信されるコロナに関する情報を受け取った人の割合" },
    { key: "trust_covid_info_govt_health", itemName: "政府から発信されるコロナに関する情報を信じる人の割合" },
    { key: "testing_rate", itemName: "最近検査を受けたか" },
    { key: "ever_tested", itemName: "検査を受けたことがあるか" },
];
...
const DisplaySurveyData: React.FC<{ display: string }> = (props) => {
    ...
    useEffect(() => {
        const fetchCountryList = async () => {
            ...
            CountryList.splice(108, 0, "UnitedStates");
    ...
    useEffect(() => {
        const fetchData = async () => {
            const ResolvedData: RawData[] = await Promise.all(FetchData(country, indicator));
            const RawData_US: RawData_US | undefined = country.includes("UnitedStates") === true ? await FetchUSData() : undefined;
            setRawData(FormatSurveyData(ResolvedData.filter(c => c.data !== null), RawData_US));
        };
        fetchData();
    }, [indicator, country]);
    return (
